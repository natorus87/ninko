...
async def processes(sort_by: str = "cpu", count: int = 10, connection_id: str = "") -> object:
    """Most active processes."""
    return await get_top_processes.ainvoke({"sort_by": sort_by, "count": count, "connection_id": connection_id})


@router.get("/services")
async def services(status_filter: str = "all", connection_id: str = "") -> object:
