...
### Fixed
- Simple cluster-status fast-path now accepts stringified `get_cluster_status`
  tool results, preventing `'str' object has no attribute 'get'` errors when
  Telegram routes Kubernetes status requests through the marketplace module.
