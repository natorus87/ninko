...
# Multilingual stopwords (de, en, fr, es, it, nl, pl, pt).
# Japanese/Chinese particles are sub-3-char and already filtered by TOKEN_RE.
# These words carry no semantic domain meaning and must not reach the resolver.
# NOTE: "send", "check", "list", "find" are intentionally NOT stopwords — they
# carry IT-ops intent and are needed for correct module routing.
_STOPWORDS = {
    # ── Ninko-intern ────────────────────────────────────────────────────────
    "ninko",
    "ninko wiki",
    ...
    "ingest",
    "ingeste",
    # ── IT-Operations-Generika (keine Modul-Semantik, alle Sprachen) ────────
    "status",   # zu generisch für Modul-Routing — ist kein Glossar-Term
    "state",
    "health",
    "report",
    "summary",
    # ── Mehrsprachig (identisch in mehreren Sprachen — einmalig geführt) ────
    "per",     # de/it/nl/en
    "con",     # es/it
    "por",     # es/pt
    "que",     # fr/es/pt
...
class SemanticResolver:
    ...
    def _overall_confidence(
        ...
        if not resolutions and unresolved_terms:
            return 0.0
        if not resolutions:
            # Keine Terme gefunden (Query bestand nur aus Stopwords) = keine Information
            return 0.0
