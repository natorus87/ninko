# Kumio Module: Telegram Bot (💬)

Das Telegram Modul ermöglicht den passiven Zugriff auf Kumio über den Telegram-Messenger.
...
1. Wähle im Kumio Backend (`⚙ Einstellungen -> Telegram`).
...
- **Polling Loop:** Kumio loggt sich asynchron über die Methode `getUpdates` bei Telegram ein. Es ist kein Webhook erforderlich. Dies ist ideal, falls Kumio tief im Intranet hinter Firewalls läuft.
- **Persistent Memory:** Kumio bindet die Chat-Historie (das Gedächtnis des LLMs) direkt an die `user_id` deines Telegram-Accounts (im Redis-Key `kumio:chat:telegram_<userid>`). Dies unterscheidet sich vom Web-GUI (wo Sessions meist ephemer sind).
...
Dies löscht das serverseitige Redis-Gedächtnis und Kumio beginnt den Chat kontextuell wieder von Null.
