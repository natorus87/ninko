...
- **Leaked tool-plan narration in chat answers** (`backend/agents/middleware/postprocess.py`, `backend/modules_catalog/kubernetes/agent.py`): the model occasionally emitted its tool-calling plan as prose ("I will call get_cluster_status to…", "I will call list_namespaces to…") directly into the final message content, with no separator before the real answer (observed run-together as `…overview.✅ Status …`). New `_strip_tool_plan_narration()` in `ResponseExtractionMiddleware` detects and strips a leading narration prefix (requires both a narration verb and ≥2 tool-call mentions in the head of the response, English and German verb-final phrasing) before the first real content marker (heading/status emoji/table row), applied for every agent. Added an explicit "never narrate tool calls" rule to the Kubernetes system prompt as the direct fix for the reported case. Regression tests cover both the exact leaked-narration string and a legitimate reply that merely opens with "I will …".
