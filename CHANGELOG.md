...
- **v1.5.3 was built from a stale `main` and silently dropped 179 commits**: the
  Knowledge-Graph release (v1.5.3) was committed directly on `main`, which had
  diverged from `feature/automation-hardening` before v1.5.0–v1.5.2 landed there.
  The deployed v1.5.3 image therefore lost the Security Core module, the
  tenant-id propagation fix (v1.5.2), the `/health` version fix (v1.5.1), the
  five built-in theme presets (Ocean, Emerald, Sunset, Crimson, Graphite) and
  the container-path fix for `BUILTIN_THEMES_DIR` — visible symptom: built-in
  themes missing from the picker and activation returning 404. This release
  merges both lines back together; it contains everything from v1.5.0–v1.5.3.
...
- **Security Core: native security-scanning module** (`backend/modules/security/`, `k8s/security/`): new built-in module for running and scheduling vulnerability, container, Kubernetes, repository, network, and AI-platform security scans, fully integrated into the existing agent runtime, workflow engine, scheduler, and safeguard system — no second/parallel agent runtime. 9 typed scanner adapters (Trivy, Kubescape, KubeLinter, Gitleaks, Checkov, Nmap, Nuclei, testssl.sh, Garak) run as isolated, least-privilege Kubernetes Jobs in a dedicated `ninko-security` namespace via `K8sJobExecutor` (non-root, read-only rootfs, capabilities dropped by default, no token automount, per-job NetworkPolicy). Scanners are invoked exclusively through typed `ExecutionSpec` argument arrays, never shell strings — command injection is structurally prevented, including against disguised `sh -c`/`bash -c` invocations. A server-side policy engine (`policy.py`) enforces target/scanner allowlists, network scope (the link-local/cloud-metadata block now applies to every network target type — IP/hostname/CIDR, not just URLs — and each scanner's job-level NetworkPolicy denies non-DNS egress by default unless a real target-derived allowlist or an explicit `mode="open"` declaration is present), and mandatory human approval for intrusive scan profiles, which can never be auto-scheduled. 9 Security Agent Profiles (7 domain specialists + Remediation + Report) delegate through the existing `DynamicAgentPool`, 5 curated multi-scanner audit workflows are available as real Ninko workflow templates with scheduler integration, and a new Security dashboard tab covers Targets/Runs/Findings. LLM finding enrichment and remediation proposals are stored separately from the original scanner output and are never auto-applied. 363 dedicated unit tests (registry/validation, agents, workflows, findings, tenant isolation/RBAC). See `backend/modules/security/CHANGELOG.md` for the module-level changelog and `.claude/memory/project_security_core.md` for architecture notes and known limitations (most notably: the target microk8s cluster's RBAC enforcement is a separate, unresolved external issue — the module's own least-privilege `Role` is correct but not yet backed by a cluster that enforces it).
...
- **Pin/favorite chat history entries** (`backend/schemas/chat.py`, `backend/api/routes_chat.py`, `frontend/app.js`, `frontend/index.html`, `frontend/style.css`): new `pinned` field on `UiHistoryEntry`, roundtripped through the existing `POST /api/chat/ui-history` upsert. Hovering a sidebar history item now reveals a star button (`.history-item-pin`) before the existing trash button; toggling it moves the entry into a new "Angepinnt" section that appears above "Zuletzt" (hidden entirely when nothing is pinned), reusing the same visual language (`--accent-favorite`) as the module-favorites star.
- **Delete entire chat history (keeps pinned entries)** (`backend/core/redis_client.py`, `backend/api/routes_chat.py`, `frontend/app.js`): new `DELETE /api/chat/ui-history` route (`ui_history_clear_all`) and a trash-can button in the sidebar's "Zuletzt" header (`clearAllHistory()`, behind a confirmation dialog) that wipes a tenant's chat history — but not entries with `pinned: true`, which is the whole point of pinning something. `ui_history_clear_all()` reads every entry, deletes only the unpinned ones via a single `HDEL`. Frontend mirrors the same filter locally and only resets to "Neuer Chat" if the currently open conversation wasn't a pinned survivor. Verified client- and server-side (survives a page reload) that a pinned entry is untouched while unpinned ones are gone.
- **Returning to Chat from Automatisierung now starts a fresh "Neuer Chat"** (`frontend/app.js`): `_doSwitchTab()` previously left whatever history entry was open still selected when navigating away to Automatisierung (and its tasks/agents/skills/workflows sub-tabs, which all route through it) and back. Now calls `newChat()` on that specific transition, so returning to the main chat area always lands on a clean compose state instead of resuming a stale conversation view.
- **Agent editor: select existing Skills and tool-enabled Scripts as agent capabilities** (`backend/schemas/agents.py`, `backend/agents/script_tools.py`, `backend/core/agent_pool.py`, `frontend/index.html`, `frontend/features/agents.js`): the Skills panel in the Agent Builder was previously read-only display only — actually attaching a skill to a custom agent meant manually editing the skill and typing the agent's name into its `modules` field. It's now a real checkbox list (`_populateAgentSkills()`); toggling a box and saving the agent diffs the selection against each skill's current `modules` (fetches full content via `GET /api/skills/{name}`, appends/removes the agent's name, `PUT`s it back) — reuses the existing `SkillsMiddleware` keyword-injection mechanism as-is, no new backend concept needed. Dynamic agents previously had **no way at all** to call script-tools (`run_script_tool`/`list_script_tools` were only wired into the main Orchestrator's tool list); a new "Scripts" panel lists every `tool_enabled` script from the Scripting module (`GET /api/scripting/tools`), and a new `script_tool_names` field on `AgentDefinition`/`AgentCreate` records the selection. Since `run_script_tool(tool_name, ...)` is a single generic dispatcher (not one tool per script), giving an agent unrestricted access to it would let it call *any* tool-enabled script regardless of what was checked — so `agent_pool.py`'s `_get_dynamic_tools()` now builds a *scoped* pair via a new `make_scoped_script_tools(allowed_tool_names)` factory in `script_tools.py` that rejects any `tool_name` outside the agent's own allow-list, only when the agent actually has scripts selected. The unscoped Orchestrator tools are unaffected (shared implementation, refactored into `_run_script_tool_impl`/`_list_script_tools_impl` helpers). Verified end-to-end via Playwright: created a tool-enabled script, selected it plus a skill in a new agent, saved, confirmed `script_tool_names` and the skill's `modules` both persisted server-side, and that reopening the editor shows both checkboxes still checked.
...
- **Login card appeared too far toward the top of the screen** (`frontend/login.html`): `body` centers `.login-shell` via `display: grid; place-items: center;` against `min-height: 100vh`. Tested across 6 desktop/laptop/widescreen/mobile viewport sizes (900×900 through 1920×1080 down to 390×844) and the card was pixel-perfectly centered in every one (0px offset), so this isn't a plain CSS centering bug on any browser viewport tested here. Most likely explanation is the well-known `100vh`-on-mobile quirk: on real mobile browsers the layout viewport used for `vh` can be taller than what's actually visible once the address bar is showing, pushing centered content up out of the visible area. Added `min-height: 100dvh` after the `100vh` line (dynamic viewport height, falls back to `100vh` on browsers that don't support it) — the standard fix for this class of issue. If the login card still appears off-center after this, it's something this couldn't reproduce headlessly and needs a screenshot/device+browser to pin down further.
- **Login screen ignored the active theme entirely** (`frontend/login.html`): `login.html` already had its own `loadUiConfig()`/`applyThemeTokens()` that fetches `/api/themes/active` and sets `tokens_dark` on `:root` — the plumbing was correct and already being called on page load. But almost the entire visible login page (aurora background glow, card gradient, input fields, focus ring, submit-button gradient, box-shadows, modal backdrop) is defined in the page's own separate inline `<style>` block, and nearly all of it hardcoded literal colors instead of referencing the CSS custom properties that `applyThemeTokens()` was busy setting — so the mechanism ran successfully but had almost nothing to act on. Converted every themeable literal in that block to reference the same tokens used by the main app (`--primary-color-rgb`, `--accent-blue-rgb`, `--bg-tint-rgb`, `--bg-surface-rgb`, `--fg-rgb`, `--shadow-rgb`, `--accent-cyan-rgb`, `--accent-blue-soft-rgb`); also made the login page's local `--danger` fallback-chain to `--status-danger` instead of a fixed hex, and bumped its `style.css` link from a long-stale `?v=66` to the current `?v=73`. Kept the eye-glow animation's cyan intentionally theme-independent, matching the same reasoning as the Denkschritte phase badges. Verified via computed styles and screenshots across default/ocean/sunset with a freshly-loaded (logged-out) session for each: card gradient, aurora glow and submit-button gradient all now track the active theme, no console errors.
