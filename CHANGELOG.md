...
- **Delete entire chat history** (`backend/core/redis_client.py`, `backend/api/routes_chat.py`, `frontend/app.js`): new `DELETE /api/chat/ui-history` route (`ui_history_clear_all`) and a trash-can button in the sidebar's "Zuletzt" header (`clearAllHistory()`, behind a confirmation dialog) that wipes the whole tenant's chat history — pinned entries included — and resets the chat view to "Neuer Chat".
