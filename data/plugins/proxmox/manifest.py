    ...
    try:
        ...
        if not conn:
            ...
            if not host:
                return {
                    "status": "ok",
                    "detail": "Keine Verbindung konfiguriert (erwartet)",
                }
            ...
            if not token_secret:
                return {
                    "status": "error",
                    "detail": "PROXMOX_TOKEN_SECRET nicht im Vault",
                }
            proxmox = ProxmoxAPI(
                host,
                user=user,
            ...
            return {
                "status": "ok",
                "detail": f"Proxmox VE {version.get('version', '?')} erreichbar (Env)",
            }
        ...
        if not token_secret or not token_id:
            return {
                "status": "error",
                "detail": f"Keine Token-Credentials für '{conn.name}'",
            }
        ...
        return {
            "status": "ok",
            "detail": f"Proxmox VE {version.get('version', '?')} erreichbar ({conn.name})",
        }
    except Exception as e:
